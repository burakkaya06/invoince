...
class DocumentService
{
    ...
    public function indexDelivery (Request $request)
    {
        ...
        return $this->prepareDeliveryContent($order , $customer , $document);
    ...
    public function indexInvoice (Request $request)
    {
        ...
        return $this->prepareInvoiceContent($order , $customer , $document);
    ...
    WHERE d.id = :documentId" , [ 'documentId' => $documentId ]);
        ...
        $document = Documents::where('id', $documentId)
            ->where('type' , DocumentType::DELIVERY)
            ->get()->toArray();
    ...
    WHERE d.id = :documentId" , [ 'documentId' => $documentId ]);
        ...
        $document = Documents::where('id', $documentId)
            ->where('type' , DocumentType::INVOINCE)
            ->get()->toArray();
    ...
    private function prepareDeliveryContent ($order , $customer, $document) {
    ...
    private function prepareInvoiceContent ($order , $customer, $document) {
    ...
    public function saveDocument (Request $request)
    {
        ...
        if ( !empty($request->products) ) {
            ...
            foreach ( $products as $product ) {
                ...
                $documentProducts->type = DocumentType::DELIVERY;
    ...
    public function saveDocumentInvoice (Request $request)
    {
        ...
        if ( $document == null ) {
            ...
            $document->order_id = $order->id;
            $document->customer_id = $customer->id;
            $document->type = DocumentType::INVOINCE;
            $document->creation_date = date('Y-m-d H:i:s' , strtotime($request->date));
            $document->delivery_date = date('Y-m-d H:i:s' , strtotime($request->deliveryDate));
            $totalAmount = str_replace('€' , '' , $request->totalAmount);
            $document->total_amount = floatval($totalAmount);
